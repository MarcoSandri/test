    ...
    <div class="header__wrapper">
      ...
      <div class="header__address t-22 t-right">
        ...
        <div class="header__block" :v-show="contact.PhoneNumber != ''">
          ...
          <p>
            <a :href="'tel:' + contact.PhoneNumber.replace(/\+|\ /g, '')">{{ contact.PhoneNumber }}</a>
          </p>
        </div>
      </div>
...
const { data: addressApi, addressPending, addressRefresh, addressError } = await useAsyncData("address", () => findOne("address"));
const address = addressApi.value.data.attributes;

const { data: contactApi, contactPending, contactRefresh, contactError } = await useAsyncData("contact", () => findOne("contact"));
  ...
  &__menu {
    &.hamburger {
      ...
      &.opened {
        ...
        span:nth-child(4),
        span:nth-child(6) {
          width: calc($iconSize / 5 * 2 + 2px);
        }
        span:nth-child(2),
        span:nth-child(8) {
          height: calc($iconSize / 5 * 2 + 2px);
        }
      }
    }
  }
